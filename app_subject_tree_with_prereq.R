...
prereq <- read.csv('bachelor_it_prerequisites.csv', stringsAsFactors = FALSE)
...
build_prereq_network <- function(subjects_data, prereq_data) {
  subject_labels <- subjects_data %>%
    mutate(label = paste(subject_code, subject_name, sep = ': ')) %>%
    select(subject_code, label)

  links_raw <- prereq_data %>%
    left_join(subject_labels, by = c('prerequisite_code' = 'subject_code')) %>%
    rename(source_name = label) %>%
    left_join(subject_labels, by = c('subject_code' = 'subject_code')) %>%
    rename(target_name = label) %>%
    filter(!is.na(source_name), !is.na(target_name)) %>%
...
ui <- fluidPage(
  titlePanel('Bachelor of IT Curriculum Explorer'),
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel(
          'Prerequisites',
          br(),
          h4('Prerequisite Network'),
          forceNetworkOutput('prereq_graph', height = '560px')
        ),
        tabPanel(
          'Subject Table',
          br(),
...
server <- function(input, output, session) {
  ...
  output$prereq_graph <- renderForceNetwork({
    data <- filtered_subjects()
    network <- build_prereq_network(data, prereq)
